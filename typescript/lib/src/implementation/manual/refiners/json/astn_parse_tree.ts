    ...
    'type': _p.decide.state($.type, ($): d_out.Value_Type => {
        switch ($[0]) {
                        ...
                        'entries': $.entries.__l_map(($) => ({
                            ...
                            'value': $.assignment.__decide(
                                ($) => $.value.__decide(
                                    ($) => Value($, abort),
                                    () => abort({
                                        'unexpected astn concept': ['missing property', null]
                                    }),
                                ),
                                () => abort({
                                    'unexpected astn concept': ['missing property', null]
                        ...
                        return _p.decide.state($.token.type, ($): d_out.Value_Type => {
                            switch ($[0]) {
                                ...
                                case 'apostrophed': return _p.ss($, ($) => abort({
                                    'unexpected astn concept': ['apostrophed text', null]
                                }))
                                        ...
                                        : x.token.value === "true"
                                            ...
                                            : x.token.value === "false"
                                                ...
                                                : ['number', r_primitives_from_loc.Number(
                                                    ...
                                                    ($) => abort({
                                                        'unexpected astn concept': ['undelimited text', null]
                                                    }),
                                                )]
                                })
                                case 'backticked': return _p.ss($, ($) => abort({
                                    'unexpected astn concept': ['backticked text', null]
                                }))
            ...
            case 'include': return _p.ss($, ($) => abort({
                'unexpected astn concept': ['include', null]
            }))
            case 'missing': return _p.ss($, ($) => abort({
                'unexpected astn concept': ['missing data', null]
            }))
            default: return _p.au($[0])
        }
    })
