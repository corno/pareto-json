...
export const Value: t_signatures.Value = ($) => _p.deprecated_cc($, ($): t_out.Value => {
    switch ($[0]) {
        case 'array': return _p.ss($, ($) => ['array', $.__l_map(($) => Value(
            $
        ))])
        case 'object': return _p.ss($, ($) => ['object', _p.deprecated_cc($, ($): t_out.Value.object_ => {
            switch ($[0]) {
                case 'key value array': return _p.ss($, ($) => ['key value array', $.__l_map(($) => ({
        ...
        case 'number': return _p.ss($, ($) => ['number', _p.deprecated_cc($, ($): t_out.Value.number_ => {
            switch ($[0]) {
                case 'integer': return _p.ss($, ($) => ['integer', $])
                case 'float': return _p.ss($, ($) => ['float', $])
                default: return _p.au($[0])
            }
        })])
        case 'string': return _p.ss($, ($) => ['string', $])
        case 'boolean': return _p.ss($, ($) => ['boolean', $])
        case 'null': return _p.ss($, ($) => ['null', null])
        default: return _p.au($[0])
    }
})
